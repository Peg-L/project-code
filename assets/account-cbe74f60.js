import{u as v}from"./backtotop-46b93ac6.js";import"./openTime-482ad5b4.js";let j=[];const te=document.querySelector("#userName"),ae=document.querySelector("#userRole");document.querySelector("#userImg");const oe=document.querySelector("#PreviousWeek"),se=document.querySelector("#NextWeek"),U=document.querySelector("#courseTeacherName"),Y=document.querySelector("#courseName"),z=document.querySelector("#dateTime");function ce(){axios.get(`http://localhost:3000/user_courses?_expand=user&userId=${v.toString()}`).then(function(e){j=e.data[0],te.textContent=j.user.name,ae.textContent=j.user.role,R()})}function R(){document.querySelectorAll(".calendar-time").forEach(i=>{let o=i.getAttribute("data-num"),n=c(o),h="";if(n.length!==0)for(let s=0;s<n.length;s++)h+=`<li><a href=''
                data-bs-toggle="modal"
                data-bs-target="#calendarModal"
                data-course-id="${n[s].courseId}"
                id="viewCourse">
                ${n[s].time}
                </a></li>`;i.innerHTML=h}),document.querySelectorAll("#viewCourse").forEach(i=>{let o=i.getAttribute("data-course-id");i.addEventListener("click",()=>{r(o)})});function c(i){return j.attendTime.filter(o=>o.date===i)}function r(i){let o=[];axios.get(`http://localhost:3000/courses/${i.toString()}?_expand=teacher`).then(function(n){o=n.data,U.textContent=o.teacher.name,Y.textContent=o.name,z.innerHTML=""})}}const ne=document.querySelectorAll("#viewCourse");ne.forEach(e=>{let t=e.getAttribute("data-course-id");e.addEventListener("click",()=>{re(t)})});function re(e){let t=[];axios.get(`http://localhost:3000/courses/${e.toString()}?_expand=teacher`).then(function(c){t=c.data,U.textContent=t.teacher.name,Y.textContent=t.name,z.innerHTML=""})}oe.addEventListener("click",()=>{R()});se.addEventListener("click",()=>{R()});ce();const le=document.querySelector(".days"),ie=document.querySelector(".current-date"),de=document.querySelectorAll(".icons span");let x=new Date().getFullYear(),m=new Date().getMonth();const ue=["一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"],X=()=>{let t=new Date(x,m,1).getDay(),c=new Date(x,m+1,0).getDate(),r=new Date(x,m,c).getDay(),i=new Date(x,m,0).getDate(),o="";for(let s=t;s>0;s--)o+=`<li class="inactive">${i-s+1}</li>`;for(let s=1;s<=c;s++){let l=s===new Date().getDate()&&m===new Date().getMonth()&&x===new Date().getFullYear()?"active":"",a=`${m+1}/${String(s).padStart(2,"0")}`;o+=`<li class="${l}" data-day="${a}">${s}</li>`}for(let s=r;s<6;s++)o+=`<li class="inactive">${s-r+1}</li>`;ie.innerText=`${ue[m]} ${x}`,le.innerHTML=o;const n=document.querySelectorAll(".days li"),h=s=>{n.forEach(l=>l.classList.remove("active")),s.classList.add("active")};n.forEach(s=>{s.addEventListener("click",l=>{h(l.currentTarget),l.target.getAttribute("data-day"),viewTimeCourse()})})};X();de.forEach(e=>{e.addEventListener("click",()=>{m=e.id==="prev"?m-1:m+1,(m<0||m>11)&&(x=e.id==="prev"?x-1:x+1,m=m<0?11:0),X()})});const he=document.querySelector("#appointment_list"),fe=document.querySelector("#course-management");let O={};function me(){axios.get(`http://localhost:3000/user_courses?userId=${v.toString()}`).then(function(e){O=[...e.data[0].purchased],manager_data=[...e.data[0].attendTime],manager_data.forEach(r=>{r.day=r.date,delete r.date}),t(O),c(manager_data);function t(r){let i="";axios.get("http://localhost:3000/courses?_expand=teacher").then(function(o){const n=o.data;r.map(l=>{const a=n.find(d=>l.courseId===d.id);return a?{...l,...a}:l}).forEach(l=>{i+=`<li class="book-card" data-courseId="${l.id}">
  ...
  </div></div>`})}else n='<p class="text-center fs-5">目前沒有追蹤任何課程</p>';followList.innerHTML=n,t(),c(),r()}catch(o){console.log("錯誤",o)}}e();function t(){const o=document.querySelector(".followPagination");let n="";if(_){const h=`<li class="page-item prevButton ${b==1?"disabled":""}"><a class="page-link" href="#" aria-label="Previous"><i class="fa-solid fa-angle-left"></i></a></li>`;n+=h;for(let l=1;l<=_;l++){let a=`<li class="page-item ${l===b?"active":""} pageButton">
