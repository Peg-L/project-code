import{c as h,a as l,i}from"./backtotop-257d1cbf.js";import"./openTime-482ad5b4.js";import{h as m}from"./startCourse-477ed2b7.js";const y=new URLSearchParams(window.location.search),d=y.get("courseId");let e=[];const b=document.querySelector("#teacherImg"),f=document.querySelector("#teacher_name"),v=document.querySelector("#course_title"),w=document.querySelector("#courseClass"),S=document.querySelector("#course_info"),x=document.querySelector("#teacher_degree"),$=document.querySelector("#teacher_experience"),q=document.querySelector("#language"),C=document.querySelector("#level"),L=document.querySelector("#intro"),P=document.querySelector("#PreviousWeek"),_=document.querySelector("#NextWeek");function k(){l.get(`http://localhost:3000/courses/${d.toString()}?_expand=teacher`).then(function(c){e=c.data,b.setAttribute("src",e.teacher.avatar),f.textContent=e.teacher.name,v.textContent=e.name,w.textContent=e.topics,S.textContent=e.info,x.textContent=e.teacher.education,$.textContent=e.teacher.experience,q.textContent=e.teacher.lang.join("/"),C.textContent=e.level,L.textContent=e.teacher.intro,u(),B()})}function u(){document.querySelectorAll(".calendar-time").forEach(r=>{let o=r.getAttribute("data-num"),t=s(o),n="";t.length!==0&&(t[0].time.forEach(a=>{t[0].useTime.find(g=>a===g)?n+=`<li><a class="text-primary" href=''>${a}</a></li>`:n+=`<li><a href=''>${a}</a></li>`}),r.innerHTML=n)});function s(r){return e.teacher.openTime.filter(o=>o.date===r)}}k();P.addEventListener("click",()=>{u()});_.addEventListener("click",()=>{u()});const p=document.querySelector(".js-appointList");m(p);async function B(){try{const c=`http://localhost:3000/coupons?courseId=${d.toString()}`,r=(await l.get(c)).data,{price:o}=e,t=[{quantity:1,price:o,duration:e.duration},...r.filter(n=>n.discountCourseNum>1).map(n=>({quantity:n.discountCourseNum,price:parseInt(o)*parseFloat(n.discount),duration:e.duration}))];M(t)}catch(c){console.log("getDiscountedPrices",c)}}function M(c){const s=/\B(?=(?:\d{3})+(?!\d))/g;let r=c.map(t=>`<li
...
</li>`,p.innerHTML=r}const D=h.replace(/html.*/,"html");l.get("http://localhost:3000/courses?_expand=teacher").then(c=>{let o=c.data.filter(a=>a.badges.includes("熱門")).slice(0,7),t="";const n=document.querySelector(".recommend-swiper");o.forEach(a=>{t+=`<div class="card teacher-card swiper-slide">
